...
def train(patterns_input,
    ...
    for i in range(iterations):
        ...
        if is_iris is not None:
            ...
            if i%100==0:
                prediction1, prediction2, prediction3 = test_iris(patterns_input, patterns_output, nn)
        errors = np.append(errors, error)
    return errors, nn, training_errors, test_errors#, prediction1, prediction2, prediction3 
...
def test_iris(inputs, targets, nn):
    ...
    for p in range(0, 5):
        x = inputs[p]
        target = targets[p,:]
        prediction1 = nn.forward_pass(x)
        print('Sample #{:02} | Target value: {:.2f} | Predicted value: {:.5f}'.format(p+1, np.argmax(target), np.argmax(prediction1)))
    for p in range(25, 30):
        x = inputs[p]
        target = targets[p,:]
        prediction2 = nn.forward_pass(x)
        print('Sample #{:02} | Target value: {:.2f} | Predicted value: {:.5f}'.format(p+1, np.argmax(target), np.argmax(prediction2)))
    for p in range(50, 55):
        x = inputs[p]
        target = targets[p,:]
        prediction3 = nn.forward_pass(x)
        print('Sample #{:02} | Target value: {:.2f} | Predicted value: {:.5f}'.format(p+1, np.argmax(target), np.argmax(prediction3)))
    print('--------------------------------------')
